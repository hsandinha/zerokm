...
import { getUserAllowedProfiles as getUserAllowedProfilesService, checkActiveSession as checkActiveSessionService } from '@/lib/services/userService';
...
export async function checkActiveSession(email: string): Promise<boolean> {
    return checkActiveSessionService(email);
}
