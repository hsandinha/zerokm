...
export function useTablesDatabase() {
    ...
    const importVeiculosFromCSV = async (
        csvData: string,
        ...
        try {
            setLoading(true);
            setError(null);
            const results = await tablesService.importVeiculosFromCSV(csvData, onProgress, options);
            return results;
        } catch (error) {
            setError('Erro ao importar veículos do CSV');
            console.error(error);
            return { success: 0, errors: [{ line: 0, reason: 'Erro interno na importação' }], rows: [] } as any;
        } finally {
            setLoading(false);
        }
    };
