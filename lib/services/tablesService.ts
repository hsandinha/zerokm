...
class TablesService {
    ...
    async importVeiculosFromCSV(
        csvData: string,
        onProgress?: (current: number, total: number) => void,
        options?: { validModelos?: string[]; validCores?: string[] }
    ): Promise<{
        success: number;
        headers?: string[];
        ...
        const validModelos = (options?.validModelos || []).map(m => m.trim().toUpperCase());
        const validCores = (options?.validCores || []).map(c => c.trim().toUpperCase());
        ...
        const results = {
            success: 0,
            headers,
            errors: [] as Array<{ line: number; reason: string; raw?: string; columns?: string[] }>,
            rows: [] as Array<{ line: number; status: 'ok' | 'error'; columns: string[]; modelo?: string; cor?: string; reason?: string }>
        };
        ...
        try {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (columns.length < 17) {
                    const reason = `Dados insuficientes - esperado 17 colunas, encontradas ${columns.length}`;
                    results.errors.push({ line: i + 1, reason, raw: line, columns });
                    results.rows.push({ line: i + 1, status: 'error', columns, reason });
                    continue;
                }
                ...
                if (!modelo || !cidade || !estado || !concessionaria || !nomeContato || !telefone) {
                    const reason = 'Campos obrigatórios em branco';
                    results.errors.push({ line: i + 1, reason, raw: line, columns });
                    results.rows.push({ line: i + 1, status: 'error', columns, modelo, cor, reason });
                    continue;
                }

                // Valida modelo contra a tabela de modelos cadastrados
                if (validModelos.length > 0) {
                    const normalizedModelo = modelo.trim().toUpperCase();
                    if (!validModelos.includes(normalizedModelo)) {
                        const reason = `Modelo "${modelo}" não encontrado na tabela de modelos. Cadastre o modelo primeiro.`;
                        results.errors.push({ line: i + 1, reason, raw: line, columns });
                        results.rows.push({ line: i + 1, status: 'error', columns, modelo, cor, reason });
                        continue;
                    }
                ...
                // Valida cor contra a tabela de cores cadastradas
                if (validCores.length > 0 && cor && cor.trim()) {
                    const normalizedCor = cor.trim().toUpperCase();
                    if (!validCores.includes(normalizedCor)) {
                        const reason = `Cor "${cor}" não encontrada na tabela de cores. Cadastre a cor primeiro.`;
                        results.errors.push({ line: i + 1, reason, raw: line, columns });
                        results.rows.push({ line: i + 1, status: 'error', columns, modelo, cor, reason });
                        continue;
                    }
                ...
                try {
                    ...
                    const normalizeStatus = (s?: string): 'A faturar' | 'Refaturamento' | 'Licenciado' | 'Pedido de fábrica' => {
                        if (!s || !s.trim()) {
                        ...
                        if (normalized === 'PEDIDO DE FABRICA' || normalized === 'PEDIDO DE FÁBRICA' || normalized === 'PEDIDO') return 'Pedido de fábrica';
                        throw new Error(`Status inválido: "${s}". Use: A FATURAR, REFATURAMENTO, LICENCIADO ou PEDIDO DE FÁBRICA`);
                    };
                    ...
                    const vehicleData = {
                        ...
                        opcionais: opcionais || '',
                        preco: preco,
                        status: statusFinal as 'A faturar' | 'Refaturamento' | 'Licenciado' | 'Pedido de fábrica',
                    ...
                    results.success++;
                    results.rows.push({ line: i + 1, status: 'ok', columns, modelo: vehicleData.modelo, cor: vehicleData.cor });
                } catch (error) {
                    const reason = `Erro ao adicionar ${modelo}: ${error}`;
                    results.errors.push({ line: i + 1, reason, raw: line, columns });
                    results.rows.push({ line: i + 1, status: 'error', columns, modelo, cor, reason });
                }
            }
